...
private readonly fb = inject(FormBuilder);
  ...
  /**
   * Build the HTML body for the "Catalog updated" toast. Same logic as the
   * add-job version. Kept duplicated rather than extracted into a service
   * because it's pure UI copy and never reused outside these two flows;
   * a service-level helper would just push the markup further from the
   * place that owns the message intent.
   */
