...
export class MyApplicationDetail implements OnInit {
  ...
  saveChanges() {
    ...
    if (githubChanged && githubUrl) {
      if (this.form.get('githubUrl')?.invalid) { this.error = 'Please enter a valid URL.'; return; }
      if (this.githubValid() === false) { this.error = 'GitHub link is broken or unreachable.'; return; }
      if (this.githubValid() === null) { this.error = 'Please click "Verify" to validate your GitHub link.'; return; }
    }
  ...
  isStepActive(step: string): boolean { return (this.app?.status || '') === step; }
  ...
  goToJob() { if (!this.app?.jobId) return; this.router.navigate(['/jobs', this.app.jobId]); }
